---
name: in-app-browser-task-queue
description: "Use the plugin-owned in-app Browser queue for up to two isolated goals, route natural work results through fresh planner Chats, recover disconnected Browser hosts, and continue until a planner verifies completion."
---

# Plugin-owned in-app Browser queue

Use `dispatch_goal` as the only outbound entry point for a long-running goal. The caller supplies one complete goal. The plugin is the controller: it creates a fresh work Chat, reads that Chat's natural-language result, creates a fresh planner/acceptance Chat, and sends the planner's raw `next_task` to the next fresh work Chat. Do not send a plan, milestone, partial result, or historical progress as a substitute for the goal.

Call `dispatch_goal` once for each independent goal. The plugin permits at most two non-terminal goals at once; each gets an isolated in-app Browser tab, a distinct persisted job id, and round-robin monitoring. Adding the second goal must not replace, resend, pause, or reuse the first goal's tab. A third goal waits for an explicit user decision after a slot becomes free.

The plugin owns the task state and completion decision. Every Chat dispatch is restricted to the in-app Browser Chat surface, GPT-5.6 Sol, Extra High reasoning, and a fresh Chat. A work Chat receives only an executable natural-language task and must never receive `MAHAYANA_TASK_REPORT_V1` or a completion/next-step template. A planner/acceptance Chat receives the original goal, the prior work Chat's natural result, and the fixed report contract; only the planner may emit `MAHAYANA_TASK_REPORT_V1`. The plugin forwards only the planner's raw `next_task` to the next work Chat. Callers must not carry historical progress or write their own continuation message. Never use Devspace, Work mode, a generic CDP endpoint, or a manually typed continuation message for this queue.

## Start or recover

1. Call `browser_watch` to start or resume the plugin's persistent supervisor from the local task state.
2. Call `browser_capability_status` and `browser_job_status` when a job id is available. `browser_capability_status` and `browser_watch` return the full `jobs` array; treat every `waiting_for_browser_host`, `reattaching_browser_host`, and `reattachRequired=true` entry as recoverable, not complete.
3. A stale tab binding is repaired inside the plugin host: it first rebinds an exact controlled conversation, then claims an exact released user tab, and only then opens a new background tab at the saved URL. It keeps the same job id and never dispatches that goal again during reattachment; other active goals continue in their own tabs.
4. If `browser_watch` returns `reattachRequired=true`, use only the authorized in-app Browser and the returned `reattach.modulePath`, `reattach.factory`, `reattach.startUrl`, `reattach.runMethod`, and optional `reattach.runOptions`. Resolve the Browser by its stable `iab` id; do not require an enumerated browser array. If discovery is empty, undefined, or temporarily unavailable during a handoff, retry with backoff and reuse the same Browser binding. Invoke that plugin factory with the existing in-app Browser binding, then **await the returned host's `runUntilTerminal()` method in the same Browser execution**. Do not start an unawaited background promise: it loses the trusted Browser context when the execution ends. For an ordinary long-lived execution, await `runUntilTerminal()` with no options. For a scheduled heartbeat or other execution with a hard time cap, pass the supplied `runOptions` (a bounded lease slice); the host persists every active job and intentionally returns with `reattachRequired=true` before the context is reclaimed, so the next heartbeat resumes the same tabs without redispatching either goal.
5. After the long-running host is attached, the plugin restores the exact saved conversation and automatically approves its authorization cards. Prefer the session-scoped option; when that option is absent, close the scope menu and click the card's direct Allow control. Browser-list failures and lease rotations are recoverable: the host retries discovery, persists the same job, and keeps pumping or reattaches without resending the goal. Every work result is handed to a fresh planner Chat; every actionable planner result is handed to a fresh work Chat. If a sent turn ends without an assistant final reply, wait only the configured bounded no-final-reply window (default five minutes), close the exact old plugin target/profile/process, and resend the same Work instruction in a fresh Chat; never hand an empty result to the planner. On a retry, stall, stop, or cancellation, close only the exact plugin-owned target/profile/process for that job before creating its replacement. The replacement may be visible or hidden; visibility is reported as runtime state and is not itself a failure. Continue until the same persisted job becomes terminal.

The long-running host bootstrap is the sole permitted recovery exception to plugin-tool-only monitoring. It is plugin-supplied reattachment, not manual page control: do not type messages, click page controls, select another browser, or construct a replacement goal outside the plugin.

## Handoff and completion

While Chat is responding, do not send another message to that tab. When a response ends, the plugin independently checks that tab's latest reply. A work result is captured as natural text and is never parsed as the terminal certificate. A planner result may be a valid complete certificate or a valid incomplete/blocked certificate with a non-empty `next_task`; the latter is actionable and starts another fresh work Chat. The task ends only when a planner Chat emits a valid `MAHAYANA_TASK_REPORT_V1` completion certificate with `status=complete`, `all_tasks_complete=true`, non-empty completion and verification evidence, empty `remaining` and `blockers`, `wait_seconds=0`, and an empty `next_task`. If a work result is missing or a planner report is malformed/missing, the plugin creates the appropriate fresh Chat and recovers the same persisted task; it never sends the report template to a work Chat or carries a stale work result as a work prompt.

The queue stops a job only after the plugin's completion detector confirms that whole goal is complete and verified, or the user explicitly stops that job. A host disconnect, plugin restart, incomplete response, authorization card, Browser/CDP/UI automation error, transient Browser error, or partial implementation must leave that task recoverable and must not be treated as completion. The host remains alive while either of its two jobs is non-terminal.

## Return to the local Work turn without polling

When the local Work agent needs one web ChatGPT answer for planning, review, or a
decision and must continue locally after that answer, use the `browser_reply_handoff`
tool after the web message has been sent. Capture the exact ChatGPT URL, the SHA-256
of the user message just sent, and the SHA-256 of the last assistant message before
sending; then register:

```json
{
  "action": "register",
  "watchId": "web-plan-1",
  "tabId": "<authorized-browser-tab>",
  "conversationUrl": "https://chatgpt.com/c/<conversation-id>",
  "expectedUserDigest": "<sha256-of-sent-user-message>",
  "baselineAssistantDigest": "<sha256-of-previous-assistant-message>"
}
```

Inside a local Work turn, `threadId` may be omitted; the local plugin host binds
the current `CODEX_THREAD_ID`. The tool accepts the registration only after an
independent local Work bridge has proved ownership. Once it reports `waiting`, end
the current Work turn immediately. Do not call `browser_watch`, read the web page
repeatedly, or issue model heartbeats while the web answer is generating. The
plugin server observes the authorized page outside the model turn, waits for two
stable seconds with no stop/retry/error/authorization state, and queues one fixed
resume message to `gpt-5.6-luna` with medium reasoning. The queue uses the supported
Codex app-server ownership check and `codex queue --thread`; the event id is
deduplicated durably and the message never includes the web answer body.

When the local Work turn wakes, read the complete web answer once, validate it, and
continue the task. A missing or changed tab, an unfinished answer, or a changed user
turn is a recoverable wait/supersession state; it is never treated as completion.
