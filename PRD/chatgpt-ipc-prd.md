...
- 后台窗口、可见窗口、隐藏会话和队列任务只通过插件自有 IPC/CDP 边界处理。任务队列可以使用插件创建的可见或隐藏独立 renderer；可见性只是运行诊断，不是安全门槛。安全边界是精确的插件端口、target、profile 和真实 Chat 表面；不得接管用户主窗口，也不得以辅助功能操作替代 target 所有权校验。
