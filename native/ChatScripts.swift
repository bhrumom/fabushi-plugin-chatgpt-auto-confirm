...
完成整个目标后，在回复末尾只输出以下完成回执；未完成时不要伪造 complete，继续工作：
...
func messageWithTaskReportContract(
  ...
  let goal = ["MAHAYANA_TASK_REPORT_CONTRACT_V5", "MAHAYANA_TASK_REPORT_V1_BEGIN"]
    .reduce(message) { current, marker in
      guard let range = current.range(of: marker) else { return current }
      return String(current[..<range.lowerBound]).trimmingCharacters(in: .whitespacesAndNewlines)
    }
