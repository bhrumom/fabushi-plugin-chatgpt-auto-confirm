...
case "send_and_watch":
  ...
  let defaultContinuationMessage = messageWithTaskReportContract(originalGoal)
  let continuationMessage = defaultContinuationMessage
  ...
  if !resumeExisting {
    let outboundMessage = messageWithTaskReportContract(originalGoal)
  ...
  if autoContinueIncomplete,
    ...
    if let report = taskReport {
      ...
      hasNextTask = true
      continuationPrompt = messageWithTaskReportContract(originalGoal)
    }
